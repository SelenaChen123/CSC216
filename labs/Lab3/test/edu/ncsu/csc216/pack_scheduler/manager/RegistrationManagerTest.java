...
public class RegistrationManagerTest {
	...
	public void testGetCourseCatalog() {
		assertNull(manager.getCourseCatalog().getCourseFromCatalog("CSC116", "002").getInstructorId());
	}
	...
	public void testResetFacultySchedule() {
		FacultyDirectory fac = manager.getFacultyDirectory();
		fac.loadFacultyFromFile("test-files/faculty_records.txt");
		CourseCatalog cat = manager.getCourseCatalog();
		cat.loadCoursesFromFile(courseFile);
		manager.resetFacultySchedule(fac.getFacultyById("lwalls"));

		manager.logout();
		manager.login("registrar", "Regi5tr@r");
		try {
			manager.resetFacultySchedule(fac.getFacultyById("lwalls"));
			assertEquals(fac.getFacultyById("lwalls").getSchedule().getNumScheduledCourses(), 0);
		} catch (IllegalArgumentException e) {
			fail();
		}
		manager.resetFacultySchedule(fac.getFacultyById("lwalls"));
	...
	/**
	 * Tests addFacultyToCourse() and removeFacultyFromCourse().
	 */
	@Test
	public void testAddRemoveFacultySchedule() {
		FacultyDirectory fac = manager.getFacultyDirectory();
		fac.loadFacultyFromFile("test-files/faculty_records.txt");
		CourseCatalog cat = manager.getCourseCatalog();
		cat.loadCoursesFromFile(courseFile);
		Course c = cat.getCourseFromCatalog("CSC116", "001");
		try {
			manager.addFacultyToCourse(c, fac.getFacultyById("awitt"));
			fail();
		} catch (IllegalArgumentException e) {
			assertEquals(fac.getFacultyById("awitt").getSchedule().getNumScheduledCourses(), 0);
		}
		...
		try {
			manager.addFacultyToCourse(c, fac.getFacultyById("awitt"));
			assertEquals(fac.getFacultyById("awitt").getSchedule().getNumScheduledCourses(), 1);
		} catch (IllegalArgumentException e) {
			fail();
		}
		Course c1 = cat.getCourseFromCatalog("CSC216", "001");
		try {
			manager.addFacultyToCourse(c1, fac.getFacultyById("awitt"));
			assertEquals(fac.getFacultyById("awitt").getSchedule().getNumScheduledCourses(), 2);
		} catch (IllegalArgumentException e) {
			fail();
		}
		try {
			manager.removeFacultyFromCourse(c, fac.getFacultyById("awitt"));
			assertEquals(fac.getFacultyById("awitt").getSchedule().getNumScheduledCourses(), 1);
		} catch (IllegalArgumentException e) {
			fail();
		}
		...
		try {
			manager.removeFacultyFromCourse(c1, fac.getFacultyById("awitt"));
			fail();
		} catch (IllegalArgumentException e) {
			assertEquals(fac.getFacultyById("awitt").getSchedule().getNumScheduledCourses(), 1);
		}
